...
let db=loadDB();
// No se crea un usuario predeterminado. El primer usuario registrado será SuperAdmin.
...
async function handler(req,res){
 ...
 if(p.startsWith('/api/')){
  try{
   ...
   if(req.method==='POST'&&p==='/api/signup'){const b=await readBody(req),email=String(b.email||'').trim().toLowerCase(),password=String(b.password||'');if(!email||password.length<6)return send(res,400,{error:'Correo y contraseña de mínimo 6 caracteres.'});if(db.users.some(x=>x.email===email))return send(res,409,{error:'El usuario ya existe.'});const user={id:id(),email,password:hash(password),name:String(b.name||email),role:db.users.length?'admin':'superadmin',created_at:new Date().toISOString()};db.users.push(user);saveDB(db);return send(res,201,{token:token(user.id),user:{id:user.id,email:user.email,name:user.name,role:user.role}})}
